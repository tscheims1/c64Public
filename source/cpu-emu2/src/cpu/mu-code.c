...
int izx_adr_mode(struct _6510_cpu* cpu, char memory[][9] ){
  int pc = pc2int(cpu);
  int ix = conv_bitstr2int(cpu->regx,0,7);
  int zp_adr_low =  conv_bitstr2int(memory[pc],0,7);
  int zp_adr_high;
  zp_adr_low = (zp_adr_low+ix) & 0xff ;
  zp_adr_high = (zp_adr_low+1) & 0xff ;
  int adr = (conv_bitstr2int(memory[zp_adr_high],0,7) <<8 ) | conv_bitstr2int(memory[zp_adr_low],0,7);
...
int izy_adr_mode(struct _6510_cpu* cpu, char memory[][9] ){
  ...
  int zpadr_high = (zpadr+1) & 0xff ;
  int hb = conv_bitstr2int(  memory[zpadr_high],0,7);
...
int abx_adr_mode(struct _6510_cpu* cpu, char memory[][9] ){
  ...
  return ( adr + conv_bitstr2int(cpu->regx,0,7) ) & 0xffff ;
}
int aby_adr_mode(struct _6510_cpu* cpu, char memory[][9] ){
  int adr = abs_adr_mode(cpu, memory);
  return ( adr + conv_bitstr2int(cpu->regy,0,7) ) & 0xffff;;
} 
...
void do_BIT(struct _6510_cpu* cpu, char *mem){
  ...
  /* if(cpu->rega[0]=='1' &&  mem[0] =='1'){ */
  /*   cpu->flags[NFLAG] = '1'; */
  /* }else{ */
  /*   cpu->flags[NFLAG] = '0'; */
  /* } */
  /* if(cpu->rega[1]=='1' &&  mem[1] =='1'){ */
  /*   cpu->flags[VFLAG] = '1'; */
  /* }else{ */
  /*   cpu->flags[VFLAG] = '0'; */
  /* } */
